return function(env)
    ...
    local function getCachedComputers()
        local valid = #cachedComputers > 0
        if valid then
    ...
    local function getCachedDoors()
        local valid = #cachedDoors > 0
        if valid then
            for _, obj in ipairs(cachedDoors) do
                if not obj or not obj.Parent then
                    valid = false
                    break
                end
        ...
        if not valid then
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") then
                    ...
                    if string.find(name, "exit", 1, true) and string.find(name, "door", 1, true) then
                        table.insert(cachedDoors, obj)
                    end
                end
            end
        end
    ...
    local function getCachedPods()
        local valid = #cachedPods > 0
        if valid then
            for _, obj in ipairs(cachedPods) do
                if not obj or not obj.Parent then
                    valid = false
                    break
                end
    ...
    local function teleportToLandmark(nameQuery)
        ...
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Model") then
                if string.find(string.lower(obj.Name), query, 1, true) then
    ...
    Library:CreateButton(TeleportPage, "Tp Map", function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            local pcs = getCachedComputers()
            if #pcs > 0 then
                local part = pcs[1]:FindFirstChildWhichIsA("BasePart") or pcs[1]
                if part then
                    char.HumanoidRootPart.CFrame = (part:IsA("Model") and part:GetPivot() or part.CFrame) + Vector3.new(0, 3, 0)
                    targetFound = true
                end
            end
            if not targetFound then
                local pods = getCachedPods()
                if #pods > 0 then
                    local part = pods[1]:FindFirstChildWhichIsA("BasePart") or pods[1]
                    if part then
                        char.HumanoidRootPart.CFrame = (part:IsA("Model") and part:GetPivot() or part.CFrame) + Vector3.new(0, 3, 0)
                        targetFound = true
                    end
